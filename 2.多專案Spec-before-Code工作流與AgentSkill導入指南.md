...
| 事件 | AI 自動執行的動作 |
| --- | --- |
| 使用者提出新功能 | Write `docs/plans/{feature}.md`（用 `_plan-template.md`），展示摘要等確認 |
| 使用者確認 Plan | 拆解為 Spec(s)，Write `docs/specs/pending/{task}.spec.md` |
| AI 準備 Edit/Write 程式碼 | 先檢查有無對應 Spec → 沒有則先建立 → 等使用者確認 → 才 Edit/Write |
| Spec 確認後開始開發 | 追加 `docs/logs/` 一條「開始開發」記錄 |
| 開發中遇到 Bug | 記入 Spec Bug Log；若有通用價值則 Write `docs/bugs/{bug}.md` |
...
5. .claude/skills/multi-project-workflow.md — 多專案協作技能（子專案關聯性由掃描結果推斷）
...
- [ ] `CLAUDE.md` 包含子專案說明（自動掃描產生）與強制性工作流協議
