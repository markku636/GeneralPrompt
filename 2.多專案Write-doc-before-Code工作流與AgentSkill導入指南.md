...
請依專案實際狀況調整  - Write-doc-before-Code 流程 ，如果有衝突抵觸，以這份文件優先，刪除重覆的。
...
| 文件類型 | 命名範例 |
| --- | --- |
| Plan | `docs/plans/20260311-001-user-auth.md` |
| Spec | `docs/specs/pending/20260311-001-add-user-crud.spec.md` |
| Bug | `docs/bugs/20260311-001-null-pointer-on-login.md` |
...
└──────┬──────────┬───────────────┘
   ...
   大型需求    中型需求
       ...
       ▼          │
┌──────────────────────────────────────────────────────────────┐
│  ① AI 自動建立 docs/plans/{YYYYMMDD}-{NNN}-{feature}.md（僅大型）             │
│     狀態：草稿 → 使用者確認 → 進行中 → 已完成                │
│     觸發：新功能 / 跨多子專案 / 架構變更                      │
└──────────┬───────────────────────────────────────────────────┘
           │ 確認後拆解     ◄──── 中型需求直接到這裡
           ▼
┌──────────────────────────────────────────────────────────────┐
│  ② AI 自動建立 docs/specs/pending/{YYYYMMDD}-{NNN}-{task}.spec.md             │
...
└──────┬──────────┬────────────────────────────────────────────┘
       ...
       │  │     若有通用價值 → 自動建立 docs/bugs/{YYYYMMDD}-{NNN}-{bug}.md      │
       │  │     永久保留，不隨 Spec 歸檔消失                    │
       │  └────────────────────────────────────────────────────┘
       │
       │ 全程自動追加
...
| 大型：使用者確認 Plan | — | 拆解為 Spec(s)，Write `docs/specs/pending/{YYYYMMDD}-{NNN}-{task}.spec.md` |
| AI 準備 Edit/Write 程式碼 | 檢查有無 Spec | 沒有 → 先建立 → 等確認 → 才 Edit/Write |
| Spec 確認後開始開發 | — | 追加 `docs/logs/` 一條「開始開發」記錄 |
| 開發中遇到 Bug | 判斷是否有通用價值 | 記入 Spec Bug Log；有通用價值 → Write `docs/bugs/{YYYYMMDD}-{NNN}-{bug}.md` |
...
2. 建立 Spec（docs/specs/pending/{YYYYMMDD}-{NNN}-{feature-name}.spec.md）
...
2. 建立 Spec（docs/specs/pending/{YYYYMMDD}-{NNN}-fix-{bug-name}.spec.md）
...
- 該功能是否已有 `docs/plans/{YYYYMMDD}-{NNN}-{feature}.md`？
- **沒有** → 自動建立 Plan（用 `_plan-template.md`），展示摘要，等使用者確認
- **有且已確認** → 進入 Step 2
- **中型需求** → 跳過此步驟，直接進入 Step 2

### Step 2: 檢查 Spec
- 該任務是否已有 `docs/specs/pending/{YYYYMMDD}-{NNN}-{task}.spec.md`？
- **沒有** → 自動建立 Spec（用 `_spec-template.md`），檔名使用日期流水號格式，展示摘要，等使用者確認
...
2. 若該 Bug 有跨專案或通用參考價值 → 自動建立 `docs/bugs/{YYYYMMDD}-{NNN}-{bug-name}.md`（用 `_bug-template.md`）
...
**步驟**:
1. **自動建立 Plan** → Write `docs/plans/{YYYYMMDD}-{NNN}-{feature}.md`，展示摘要等確認
2. 使用者確認 Plan 後 → **自動拆解為 Spec(s)** → Write `docs/specs/pending/{YYYYMMDD}-{NNN}-{task}.spec.md`
...
**觸發**: 使用者要求新增或修改資料表
**步驟**:
1. **自動建立 Spec** → Write `docs/specs/pending/{YYYYMMDD}-{NNN}-{task}.spec.md`（若有關聯 Plan 則自動連結）
...
2. 判斷影響範圍（是否跨子專案）
3. **自動建立 Spec** → Write `docs/specs/pending/{YYYYMMDD}-{NNN}-fix-{bug}.spec.md`
4. 使用者確認後修復 → **自動寫入開發日誌**
5. 修復完成 → **自動記入 Spec Bug Log**
6. 若 Bug 有通用價值 → **自動建立** `docs/bugs/{YYYYMMDD}-{NNN}-{bug}.md`
7. 完成後 → **自動歸檔** Spec，追加 Log 完成記錄
...
1. 分析問題點
2. 評估影響範圍（grep 所有引用點）
3. **自動建立 Plan** → Write `docs/plans/{YYYYMMDD}-{NNN}-refactor-{module}.md`
...
- **位置**: `docs/specs/pending/{YYYYMMDD}-{NNN}-{task-name}.spec.md`
...
2. 在 `docs/specs/pending/` 建立 `{YYYYMMDD}-{NNN}-{task-name}.spec.md`（流水號自動遞增）
...
> 建立日期: {YYYY-MM-DD}
> 狀態: 🟡 待確認 / 🔵 開發中 / ✅ 已完成
> 關聯計劃書: {docs/plans/{YYYYMMDD}-{NNN}-xxx.md 或「無」}
...
> 相關 Spec: {docs/specs/completed/{YYYYMMDD}-{NNN}-xxx.spec.md 或「無」}
> 影響範圍: {受影響的子專案}
...
| Spec 檔名 | 狀態 | 說明 |
| --- | --- | --- |
| `docs/specs/pending/{YYYYMMDD}-{NNN}-{task-1}.spec.md` | 🟡/🔵/✅ | {說明} |
| `docs/specs/pending/{YYYYMMDD}-{NNN}-{task-2}.spec.md` | 🟡/🔵/✅ | {說明} |
...
> 關聯 Plan: {docs/plans/{YYYYMMDD}-{NNN}-xxx.md 或「無」}
> 關聯 Spec: {docs/specs/pending/{YYYYMMDD}-{NNN}-xxx.spec.md 或「無」}
 ...
 *
 * 在 Edit/Write 程式碼檔案前，檢查 docs/specs/pending/ 是否已有
...
function main() {
  ...
  const specsDir = join(projectDir, "docs", "specs", "pending");

  // docs/specs/pending/ 不存在 → 攔截
  if (!existsSync(specsDir)) {
    process.stderr.write(
      "⛔ Spec-before-Code: docs/specs/pending/ 目錄不存在。\n" +
      "請先建立 Spec 再修改程式碼。\n"
    );
    process.exit(2);
  }
