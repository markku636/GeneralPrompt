    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash \"$CLAUDE_PROJECT_DIR/.claude/hooks/spec-before-code.sh\"",
            "timeout": 10
          }
        ]
      }
...
#### `.claude/hooks/spec-before-code.sh`

```bash
#!/bin/bash
# Spec-before-Code 攔截 Hook
# 在 Edit/Write 程式碼檔案前，檢查是否已有確認中（🔵）的 Spec
...
# 定義程式碼檔案的副檔名
CODE_EXTENSIONS="ts tsx js jsx py cs go java rb rs cpp c h swift kt vue svelte php"
...
# 非程式碼檔案（.md, .json, .yaml 等）→ 直接放行
if [ "$IS_CODE" = false ]; then
  exit 0
fi

# 檢查 docs/specs/pending/ 目錄下是否有狀態為 🔵 的 Spec
SPECS_DIR="$CLAUDE_PROJECT_DIR/docs/specs/pending"

if [ ! -d "$SPECS_DIR" ]; then
  echo "⛔ Spec-before-Code: docs/specs/pending/ 目錄不存在。請先建立 Spec 再修改程式碼。" >&2
  exit 2
fi

# 搜尋是否有任何包含 🔵 狀態的 Spec 檔案
ACTIVE_SPEC=$(grep -rl "🔵" "$SPECS_DIR"/*.spec.md 2>/dev/null | head -1)

if [ -z "$ACTIVE_SPEC" ]; then
  echo "⛔ Spec-before-Code: 找不到狀態為 🔵（開發中）的 Spec。" >&2
  echo "請先建立 Spec（/create-spec）並確認後再修改程式碼。" >&2
  echo "修改目標: $FILE_PATH" >&2
  exit 2
fi
...
> **注意**: 此腳本需要 `jq` 工具。可依環境改用 Python 版本（見下方）。
...
```python
#!/usr/bin/env python3
"""Spec-before-Code 攔截 Hook（Python 跨平台版）"""
...
CODE_EXTENSIONS = {
    '.ts', '.tsx', '.js', '.jsx', '.py', '.cs', '.go', '.java',
    '.rb', '.rs', '.cpp', '.c', '.h', '.swift', '.kt', '.vue',
    '.svelte', '.php'
}
...
def main():
    ...
    # 檢查 docs/specs/pending/ 是否有 🔵 狀態的 Spec
    project_dir = os.environ.get('CLAUDE_PROJECT_DIR', os.getcwd())
    specs_dir = Path(project_dir) / 'docs' / 'specs' / 'pending'

    if not specs_dir.is_dir():
        print("⛔ Spec-before-Code: docs/specs/pending/ 目錄不存在。"
              "請先建立 Spec 再修改程式碼。", file=sys.stderr)
        sys.exit(2)

    # 搜尋包含 🔵 的 .spec.md 檔案
    active_specs = [
        f for f in specs_dir.glob('*.spec.md')
        if '🔵' in f.read_text(encoding='utf-8', errors='ignore')
    ]

    if not active_specs:
        print(f"⛔ Spec-before-Code: 找不到狀態為 🔵（開發中）的 Spec。\n"
              f"請先建立 Spec（/create-spec）並確認後再修改程式碼。\n"
              f"修改目標: {file_path}", file=sys.stderr)
        sys.exit(2)
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 \"$CLAUDE_PROJECT_DIR/.claude/hooks/spec-before-code.py\"",
            "timeout": 10
          }
        ]
      }
...
jq -n '{
  hookSpecificOutput: {
    ...
    permissionDecisionReason: "尚無開發中的 Spec。請先執行 /create-spec 建立技術規格文件。"
  }
}'
exit 0
```
...
| **設定難度** | 低 — 只需撰寫 Markdown 規則 | 中 — 需撰寫 Bash/Python 腳本 |
...
```text
請幫我在這個專案設定 Claude Code Hook，強制 Spec-before-Code：

1. 建立 .claude/hooks/spec-before-code.sh（或 .py）
...
3. 確保 docs/specs/pending/ 目錄存在

請根據這個專案的作業系統環境選擇 Bash 或 Python 版本。
```
...
- [ ] `.claude/hooks/spec-before-code.sh`（或 `.py`）存在且有執行權限
- [ ] `.claude/settings.json` 包含 `hooks.PreToolUse` 設定
