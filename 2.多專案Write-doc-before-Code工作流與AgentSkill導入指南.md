...
10. docs/knowledge/_knowledge-template.md — Knowledge 知識提煉模板
11. 各目錄的 README.md
...
- AI 建立 Plan/Spec 前必須先查閱 docs/knowledge/ 既有知識，避免重複踩坑
```
...
- 掃描 `docs/knowledge/` 各子目錄（architecture/、patterns/、domain/、integrations/），找出與當前任務相關的既有知識
- 將相關知識納入 Plan/Spec 的規劃考量，避免重複踩坑或違反已知慣例
- 若找到相關知識，在 Plan/Spec 的「背景」區塊引用來源
...
**步驟**:
1. **查閱知識庫** → 掃描 `docs/knowledge/` 找出與需求相關的既有知識，納入規劃考量
...
**觸發**: 使用者要求新增或修改資料表
**步驟**:
1. **查閱知識庫** → 掃描 `docs/knowledge/` 找出相關架構/模式知識
2. **自動建立 Spec** → Write `docs/specs/doing/{YYYYMMDD}-{NNN}-{task}.spec.md`（若有關聯 Plan 則自動連結）
...
8. **知識提煉** → AI 判斷是否有架構/模式知識值得提煉 → 有則建立/更新 `docs/knowledge/{category}/`
...
- [ ] 實測知識提煉：Spec 完成後，AI 自動判斷是否有可複用知識並建立/更新 Knowledge 文件
- [ ] 實測知識更新：已有相關 Knowledge 時，AI 更新既有文件而非建立重複文件
- [ ] 實測知識跳過：無可複用知識時，AI 不會強制產生 Knowledge 文件
...
> **注意**：SessionEnd Hook 不處理 `docs/knowledge/`。Knowledge 文件由 AI 在開發過程中直接建立，不需歸檔。
