...
│   ├── hooks/                             # Hook 腳本（硬性攔截）
│   │   └── spec-before-code.mjs           # Spec-before-Code 攔截（Node.js ESM）
...
| 任務完成 | — | 移 Spec/Bug 至各自 `completed/`；更新 Plan 狀態；追加 Log「完成」記錄 |
...
- SessionEnd Hook 自動歸檔：Session 結束時，自動掃描所有 doing/ 資料夾，將 ✅ 已完成的文件移至 completed/
...
- **沒有 Plan** → 自動建立 Plan 至 `doing/`（用 `_plan-template.md`），展示摘要，等使用者確認
- **有且已確認** → 進入 Step 2
- **中型需求**（且無參考資料）→ 跳過此步驟，直接進入 Step 2

### Step 2: 檢查 Spec
- 該任務是否已有 `docs/specs/doing/{YYYYMMDD}-{NNN}-{task}.spec.md`？
- **沒有** → 自動建立 Spec 至 `doing/`（用 `_spec-template.md`），檔名使用日期流水號格式，展示摘要，等使用者確認
...
**步驟**:
1. **自動建立 Plan** → 若使用者提供了參考資料（`references/`），先用 Read tool 讀取所有檔案（PDF/MD/SQL 等，Claude Code 原生支援讀取 PDF），再依據內容建立 `docs/plans/doing/{YYYYMMDD}-{NNN}-{feature}/plan.md`；無參考資料時建立單檔 `docs/plans/doing/{YYYYMMDD}-{NNN}-{feature}.md`。展示摘要等確認
2. 使用者確認 Plan 後 → **自動拆解為 Spec(s)** → Write `docs/specs/doing/{YYYYMMDD}-{NNN}-{task}.spec.md`
...
**觸發**: 使用者要求新增或修改資料表
**步驟**:
1. **自動建立 Spec** → Write `docs/specs/doing/{YYYYMMDD}-{NNN}-{task}.spec.md`（若有關聯 Plan 則自動連結）
...
2. 判斷影響範圍（是否跨子專案）
3. **自動建立 Spec** → Write `docs/specs/doing/{YYYYMMDD}-{NNN}-fix-{bug}.spec.md`
4. 使用者確認後修復 → **自動寫入開發日誌**
...
1. 分析問題點
2. 評估影響範圍（grep 所有引用點）
3. **自動建立 Plan** → Write `docs/plans/doing/{YYYYMMDD}-{NNN}-refactor-{module}.md`
...
- [ ] `.claude/hooks/session-end-archive.mjs` 存在（SessionEnd 自動歸檔）
...
```javascript
/**
 * Spec-before-Code 攔截 Hook（Node.js ESM 跨平台版）
 *
 * 在 Edit/Write 程式碼檔案前，檢查 docs/specs/doing/ 是否已有
 * 狀態為 🔵（開發中）的 Spec。沒有就攔截，有就放行。
 *
 * 使用方式：node spec-before-code.mjs（透過 Claude Code Hook 自動呼叫）
 */
...
function main() {
  ...
  if (!input) process.exit(0); // 無法解析 → 放行
  ...
  if (!filePath) process.exit(0); // 無檔案路徑 → 放行

  // 非程式碼檔案（.md, .json, .yaml 等）→ 放行
  ...
  const hasActiveSpec = specFiles.some((f) => {
...
- [ ] `.claude/hooks/spec-before-code.mjs` 存在
- [ ] `.claude/hooks/session-end-archive.mjs` 存在
- [ ] `.claude/settings.json` 包含 `hooks.PreToolUse` 與 `hooks.SessionEnd` 設定
- [ ] 實測攔截：無 🔵 Spec 時修改 `.ts` 檔案 → 被攔截
- [ ] 實測放行：有 🔵 Spec 時修改 `.ts` 檔案 → 正常執行
- [ ] 實測豁免：修改 `.md` / `.json` 檔案 → 不受攔截
- [ ] 實測 SessionEnd 歸檔：`doing/` 中 ✅ 的文件被自動移至 `completed/`
...
```javascript
/**
 * SessionEnd 自動歸檔 Hook（Node.js ESM 跨平台版）
 *
 ...
 *
 * 使用方式：由 Claude Code SessionEnd Hook 自動呼叫
 */

import { readFileSync, readdirSync, renameSync, existsSync, mkdirSync, statSync } from "node:fs";
...
function main() {
  ...
  for (const { doing, completed } of ARCHIVE_DIRS) {
    ...
    const entries = readdirSync(doingDir);
    ...
    for (const entry of entries) {
      ...
      // 檢查是否包含已完成標記
      const isCompleted = COMPLETED_MARKERS.some((marker) =>
        content.includes(marker)
      );

      if (isCompleted) {
        const destPath = join(completedDir, entry);
        renameSync(srcPath, destPath); // 單檔或整個資料夾（含 references/）
        archived.push(`${doing}/${entry} → ${completed}/${entry}`);
      }
    }
  }
...
### 一鍵導入 SessionEnd Hook 的 Prompt

```text
請幫我在這個專案設定 Claude Code SessionEnd Hook，自動歸檔已完成的文件：

1. 建立 .claude/hooks/session-end-archive.mjs（Node.js ESM，Windows / macOS 通用）
   - Session 結束時掃描 docs/plans/doing/、docs/specs/doing/、docs/bugs/doing/
   - 讀取每個 .md 檔案，檢查是否包含 ✅ 標記
   - 包含 → 移至對應 completed/ 資料夾
   - 不包含 → 保留在 doing/
...
2. 在 .claude/settings.json 加入 SessionEnd hooks 設定
   - command: node "$CLAUDE_PROJECT_DIR/.claude/hooks/session-end-archive.mjs"
