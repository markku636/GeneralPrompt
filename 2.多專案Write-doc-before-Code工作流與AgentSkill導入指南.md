...
- [ ] `.claude/hooks/spec-before-code.mjs` 存在（PreToolUse 精準路徑比對攔截）
- [ ] `.claude/hooks/post-tool-use-spec-tracker.mjs` 存在（PostToolUse 實際變更追蹤）
- [ ] `.claude/hooks/session-end-archive.mjs` 存在（SessionEnd 自動歸檔 + manifest）
- [ ] `.claude/hooks/session-end-knowledge.mjs` 存在（SessionEnd 知識庫提煉 + INDEX.md）
...
**檔案存在**
- [ ] `.claude/hooks/session-start-brief.mjs` 存在
- [ ] `.claude/hooks/spec-before-code.mjs` 存在（v2 精準路徑比對版）
- [ ] `.claude/hooks/post-tool-use-spec-tracker.mjs` 存在
- [ ] `.claude/hooks/session-end-archive.mjs` 存在（含 manifest 輸出）
- [ ] `.claude/hooks/session-end-knowledge.mjs` 存在（讀 manifest + INDEX.md）
