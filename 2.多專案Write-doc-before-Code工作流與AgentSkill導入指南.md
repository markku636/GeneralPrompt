...
- archive manifest（docs/.last-session-archive.json）為暫存檔，加入 .gitignore，每次 Session 覆蓋
...
- [ ] `.gitignore` 包含 `docs/.last-session-archive.json`
...
- [ ] 實測 SessionEnd 歸檔：`doing/` 中 ✅ 的文件移至 `completed/`，`docs/.last-session-archive.json` 被建立
...
>
> **Last-Session Archive**：`docs/.last-session-archive.json` 記錄**本次** SessionEnd 歸檔的檔案清單（含 type 欄位），由 `session-end-knowledge.mjs` 讀取使用。此檔每次 SessionEnd 整檔覆寫，僅保留本次 Session 結果，並非累積歷史 — 歷史歸檔紀錄看 `completed/` 資料夾的 `git log`。檔案應加入 `.gitignore`。
...
4. .claude/hooks/session-end-archive.mjs（SessionEnd，第一個執行）
   ...
   - 完成後寫入 docs/.last-session-archive.json 記錄本次歸檔清單（含 type 欄位：requirement/plan/spec/bug）

5. .claude/hooks/session-end-knowledge.mjs（SessionEnd，第二個執行）
   - 讀取 .last-session-archive.json 取得本次歸檔的 Spec/Bug（過濾掉 requirement/plan）
...
8. 在 .gitignore 加入 docs/.last-session-archive.json（此為每次 Session 覆蓋的暫存檔）
```
